...
public class MatMulTest {
    ...
    @Test void matmul1D() {
        // 1D x 1D → dot product in 1x1 array
        try (var a = Arena.ofConfined()) {
            NdArray r = MatMul.matmul(NdArrayFactory.array(a, new float[]{1,2,3}), NdArrayFactory.array(a, new float[]{4,5,6}));
            assertEquals(1, r.size());
            assertEquals(32f, r.flatGetFloat(0), T);
        }
    }

    @Test void matmulLargeSimd() {
        // Large enough to exercise SIMD vector loop in simdMatmul
        try (var a = Arena.ofConfined()) {
            int n = 32;
            float[] data = new float[n * n];
            for (int i = 0; i < n; i++) data[i * n + i] = 1f; // identity
            NdArray eye = NdArrayFactory.array(a, data, n, n);
            float[] bData = new float[n * n];
            for (int i = 0; i < n * n; i++) bData[i] = i;
            NdArray B = NdArrayFactory.array(a, bData, n, n);
            NdArray r = MatMul.matmul(eye, B);
            assertEquals(bData[0], r.flatGetFloat(0), T);
            assertEquals(bData[n * n - 1], r.flatGetFloat(n * n - 1), T);
        }
    }
}
