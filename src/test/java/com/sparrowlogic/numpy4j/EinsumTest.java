...
public class EinsumTest {
    ...
    @Test void elementwiseDot() { try (var a = Arena.ofConfined()) { assertEquals(70f, Einsum.einsum("ij,ij->", NdArrayFactory.array(a, new float[]{1,2,3,4}, 2, 2), NdArrayFactory.array(a, new float[]{5,6,7,8}, 2, 2)).flatGetFloat(0), T); } }
    @Test void generalContraction() { try (var a = Arena.ofConfined()) { assertArr("ik,kj->ij", Einsum.einsum("ik,kj->ij", NdArrayFactory.array(a, new float[]{1,2,3,4}, 2, 2), NdArrayFactory.array(a, new float[]{5,6,7,8}, 2, 2)), new float[]{19,22,43,50}); } }
}
