...
public class IndexOpsTest {
    ...
    @Test void argpartition() { try (var a = Arena.ofConfined()) { NdArray r = IndexOps.argpartition(NdArrayFactory.array(a, new float[]{3,1,4,1,5,9,2,6}), 3); assertEquals(8, r.size()); } }
    @Test void fancyIndex() { try (var a = Arena.ofConfined()) { assertArr("fi", IndexOps.fancyIndex(NdArrayFactory.array(a, new float[]{10,20,30,40,50}), new int[]{0,2,4}), new float[]{10,30,50}); } }
}
