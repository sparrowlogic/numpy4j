...
public final class ShapeUtils {
    ...
    public static int[] cStrides(int[] shape) {
    ...
    public static long flatIndex(int[] indices, int[] strides) {
        long idx = 0;
        for (int i = 0; i < indices.length; i++) idx += (long) indices[i] * strides[i];
        return idx;
    }

    /** Convert flat index to multi-dimensional indices. */
    public static int[] unravelIndex(long flatIdx, int[] shape) {
        int[] indices = new int[shape.length];
        for (int i = shape.length - 1; i >= 0; i--) {
            indices[i] = (int) (flatIdx % shape[i]);
            flatIdx /= shape[i];
        }
        return indices;
    }
    ...
    public static int[] broadcastStrides(int[] shape, int[] strides, int[] targetShape) {
